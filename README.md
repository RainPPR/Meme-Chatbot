...
- **沉浸式 AI 对话界面**：采用类似主流现代大模型产品的清爽设计，内置完备的暗色 / 亮色主题切换能力，支持基于 HTML class 的瞬时响应与本地存储记忆。
- **作梗响应引擎与流式打字动效**：
  - **高质量抗聚集随机算法**：采用 Web Crypto API 加密级均匀分布配合滑动窗口抗聚集采样器（`AntiClusteringPicker`），杜绝原生 `Math.random()` 引起的回复碰撞与连续重复抽中同一梗的问题。
  - 无论用户提问任何内容，均智能从预设的经典梗回复列表中均衡抽取回答。
  - 拟真展示「思考中...」动画（均匀 0.5s ～ 1.5s 延迟）。
  - **极速流式打字动效**：思考完成后，以每 10ms～20ms 吐出 5～10 个字符的高频节奏进行平滑流式输出，纯粹模拟大模型 Token 生成流，无多余人类停顿；打字过程附带光标闪烁提示，并支持随时点击停止生成。
